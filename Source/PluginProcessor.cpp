...
SynthFrameworkAudioProcessor::SynthFrameworkAudioProcessor()
	...
	tree (*this, nullptr,"PARAMETERS",
		{
			std::make_unique<AudioParameterFloat>("attack", "Attack", NormalisableRange<float>(0.1f, 5000.0f), 1.0f),
			std::make_unique<AudioParameterFloat>("decay", "Decay", NormalisableRange<float>(0.1f, 2000.0f), 1.0f),
			std::make_unique<AudioParameterFloat>("sustain", "Sustain", NormalisableRange<float>(0.0f, 1.0f), 1.0f),
			std::make_unique<AudioParameterFloat>("release", "Release", NormalisableRange<float>(0.1f, 5000.0f), 1.0f),
			std::make_unique<AudioParameterFloat>("wavetype", "WaveType", NormalisableRange<float>(0.0f, 2.0f, 1.0f), 0.0f),
			std::make_unique<AudioParameterFloat>("filterType", "filterType", NormalisableRange<float>(0.0f, 2.0f, 1.0f), 0.0f),
			std::make_unique<AudioParameterFloat>("filterCutoff", "FilterCutoff", NormalisableRange<float>(20.0f, 10000.0f, 1.0f, skewFromCenterPoint(20.0f,10000.0f,1000.0f)), 400.0f),
			std::make_unique<AudioParameterFloat>("filterRes", "FilterResonance", NormalisableRange<float>(1.0f, 5.0f),1.0f),
		})
#endif
{
...
void SynthFrameworkAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
	...
	for (int i = 0; i < mySynth.getNumVoices(); i++)
	{
		if (myVoice = dynamic_cast<SynthVoice*>(mySynth.getVoice(i)))
		{
			...
			myVoice->getOscillatorType(tree.getRawParameterValue("wavetype"));

			// TODO - Investigate why sound cuts out completely past ~8500Hz
			//Logger::outputDebugString(std::to_string(*tree.getRawParameterValue("filterCutoff")));

			myVoice->getFilterParams(
				tree.getRawParameterValue("filterType"),
...
ValueType SynthFrameworkAudioProcessor::skewFromCenterPoint(ValueType start, ValueType end, ValueType centrePointValue)
{
	return static_cast<ValueType>(std::log(static_cast<ValueType> (0.5)) / std::log((centrePointValue - start) / (end - start)));
}